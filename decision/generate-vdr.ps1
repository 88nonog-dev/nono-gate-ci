...
$d=Get-Content -LiteralPath $decisionFile -Raw -Encoding UTF8 | ConvertFrom-Json
...
$ledgerMerkle=(Get-Content -LiteralPath $ledgerMerkleFile -Raw -Encoding UTF8).Trim()
...
$s=Get-Content -LiteralPath $sarifPath -Raw -Encoding UTF8 | ConvertFrom-Json
...
if(Test-Path $consFile){
  try{ $c=Get-Content -LiteralPath $consFile -Raw -Encoding UTF8 | ConvertFrom-Json; $consensusMode=$c.rule; $consensusReason=$c.reason } catch { $consensusMode=""; $consensusReason="" }
}
